...
export class StripePaymentService implements PaymentServicePort {
  public async createPaymentIntent(input: CreatePaymentIntentInput): Promise<Result<PaymentIntentResult, PaymentError>> {
    try {
      // Use external payment service API
      const paymentServiceUrl = process.env.NEXT_PUBLIC_PAYMENT_SERVICE_URL || 'http://localhost:3002';
      const response = await fetch(`${paymentServiceUrl}/api/payments/create-intent`, {
        method: 'POST',
        headers: { 
  ...
  public async getPaymentStatus(paymentIntentId: string): Promise<Result<PaymentStatusResult, PaymentError>> {
    try {
      // Use external payment service API
      const paymentServiceUrl = process.env.NEXT_PUBLIC_PAYMENT_SERVICE_URL || 'http://localhost:3002';
      const response = await fetch(`${paymentServiceUrl}/api/payments/status/${paymentIntentId}`, {
        method: 'GET',
        headers: { 
          'Accept': 'application/json'
        }
