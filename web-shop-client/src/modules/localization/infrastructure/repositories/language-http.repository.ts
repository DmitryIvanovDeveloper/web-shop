  ...
  async getActiveLanguage(): Promise<Result<LanguageResponse | null, Error>> {
    try {
      ...
      if (response.status !== 200) {
        ...
        return Result.error(new Error(response.statusText || 'Failed to get active language'));
      }
      ...
      this._logger.info('[LanguageHttpRepository] Active language found', { languageCode: (data as LanguageResponse).code });
        return Result.ok(data as LanguageResponse);
    } catch (error) {
  ...
  async getAllLanguages(): Promise<Result<LanguageResponse[], Error>> {
    try {
      ...
      if (response.status !== 200) {
        ...
        return Result.error(new Error(response.statusText || 'Failed to get all languages'));
      }
      ...
      return Result.ok(languages as LanguageResponse[]);
    } catch (error) {
  ...
  async setActiveLanguage(code: string): Promise<Result<void, Error>> {
    try {
      ...
      if (response.status !== 200) {
        ...
        return Result.error(new Error(response.statusText || 'Failed to activate language'));
      }
